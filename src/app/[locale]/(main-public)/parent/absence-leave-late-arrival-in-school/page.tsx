...
const AbsenceLeaveLateArrivalInSchoolPage: FC<NextPageProps> = ({
  ...
  return (
    <ArticlePage linkType="parent" selected={{ translationKey: "absence" }}>
      ...
      <p>
        Parents and guardians will appreciate that absence during the term is
        not in the best interest of the student. They are asked not to apply for
        leave if avoidable, but a proper leave application has to be submitted
        under any circumstances. Leave can be granted to the student only on the
        following grounds:
      </p>
      <ArticleList list={leaveConditions} containerClassName="space-y-1.5" />
      ...
      <StaticTable
        ...
        headerRow={{
          cells: [{ text: "Disease" }, { text: "When should he/she appear?" }],
        }}
      ...
      <ArticleList list={lateArrivalRules} containerClassName="space-y-1" />

      <ArticleHeading>Withdrawals</ArticleHeading>
      <ArticleList list={withdrawalRules} containerClassName="space-y-1" />
    </ArticlePage>
  );
};
